...
function isInner(node) {
	return node.indexOf(SEQ) == 0 || node.indexOf(SEL) == 0;
}
...
function onSetup() {
	...
	ga.crossover = function(l, r) {
		...
		l.root.splice(idx1, end1-idx1-1)

		var idx2 = nthAttPoint(r.root, p2);
		var end2 = findInsertionPoint(r.root, idx2, innerCount(r.root))

		// insert new subtree
		flatInsert(l.root, r.root.slice(idx2, end2))
		return {"root": l.root, "attPoints": countAttPoints(l.root)};
	}
	...
	var res = ga.optimise(50, 0.1, 0.6, 0.05)
	print(res.root)
	print(res.attPoints)
}
