...
function overlap(l,r) {
	var smaller = l.length < r.length ? l : r;
	var maxLen = smaller.length
	var missing = 0
	for(var i = 0; i < smaller.length; ++i) {
		if(r.indexOf(smaller[i]) == -1) {
			missing++
		}
	}
	return (maxLen - missing) / maxLen
}
...
function findBestTree(ngrams, actions, pcSet, trigrams, conditions, allSolutions, goal, params) {
	...
	ga.crossover = function(l, r) {
		...
		var idx1 = Random.uniformInt(0, l.root.length - 1)
		var end1;
		if(isInner(l.root[idx1])) {
			var ic1  = innerCount(l.root[idx1]);
			end1 = findInsertionPoint(l.root, idx1, ic1)
		} else {
			end1 = idx1 + 1;
		}
	...
	ga.mutation = function(c) {
		...
		var string = c.root.slice();
		var idx = Random.uniformInt(0, string.length - 1)

		if(isInner(string[idx])) {
			var val = string[idx]
			val[1] = val[1] == 'L' ? 'S' : 'L'
		} else if(isCondition(string[idx])) {
			var whichPC = Random.uniformInt(0, pcSet.length - 1);
			var pc = pcSet[whichPC]
			var cond = string[idx]
			cond[pc.name] = pc.value
		} else /*isAction*/ {
			var actionKeys = getKeys(actions)
			string[idx] = actionKeys[Random.uniformInt(0, actionKeys.length - 1)];
		}

		return {"root": string, "attPoints": c.attPoints}
	}
/*
	ga.mutation = function(c) {
		if(Random.uniform() < 0.35) {
	...
	ga.generator = function(idx) {
		var numSeqs = Random.uniformInt(2, ngrams.length - 1);
		var attPoints = 1;
		var root = ['[L0'];
